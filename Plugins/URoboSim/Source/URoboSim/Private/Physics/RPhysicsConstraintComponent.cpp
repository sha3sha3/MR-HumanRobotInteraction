...
{
  SetMotorJointStateInUUnits(-FConversions::MToCm((float)ClampJointStateToConstraintLimit(TargetPosition)), -TargetJointVelocity * 100.f);
}
...
void URPrismaticConstraintComponent::SetJointPosition(float Angle, FHitResult * OutSweepHitResult)
{
  ...
  FVector DeltaJointLocationInJointFrame = RefAxis * (Angle * 100.f);
